...
export default function MemoryListEditor({ field, value, onChange }) {
    ...
    return (
        <div data-testid={EDITOR.field.memoryList(field.key)} className="space-y-3">
            {items.map((item, i) => {
                ...
                return (
                    <div
                        key={i}
                        data-testid={EDITOR.field.memoryItem(field.key, i)}
                        className="lws-card"
                    >
                        <div className="flex items-center gap-2 p-3">
                            ...
                            <button
                                type="button"
                                onClick={() => move(i, -1)}
                                data-testid={EDITOR.field.memoryItemUp(field.key, i)}
                            ...
                            <button
                                type="button"
                                onClick={() => move(i, 1)}
                                data-testid={EDITOR.field.memoryItemDown(field.key, i)}
                                className="p-2 rounded-md border border-[color:var(--lws-border-strong)] text-[color:var(--lws-text-muted)] hover:text-[color:var(--lws-pink)]"
                            ...
                            <button
                                type="button"
                                onClick={() => remove(i)}
                                data-testid={EDITOR.field.memoryItemRemove(field.key, i)}
            ...
            <button
                type="button"
                onClick={add}
                data-testid={EDITOR.field.memoryAdd(field.key)}
                disabled={items.length >= max}
                className="lws-btn-ghost text-xs disabled:opacity-40"
            >
                <Plus size={14} /> Add memory {items.length}/{max}
            </button>
        </div>
    );
}
